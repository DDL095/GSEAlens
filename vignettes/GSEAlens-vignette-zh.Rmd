...
- **DotPlot 面板**：横向点图。横轴为 NES，点颜色编码显著性
  ...
  基因集量级。尺寸映射使用**固定域 + sqrt 感知**（而非每次绘制 min-max

  归一化），这样切换"Core Genes Count"（leading edge 数）与"Set Size"

  （MSigDB 基因集总数）时点的视觉形态会有明显差异，且跨参数可比较。这与

  `enrichplot::dotplot` 使用的 `ggplot2::scale_size_continuous(range=...)`
