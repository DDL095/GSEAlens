...
preproc_limma <- readRDS(system.file(
...
dds_se <- readRDS(system.file(

  "extdata", "preprocessed_dds_se.rds", package = "GSEAlens"

))

dds    <- readRDS(system.file(

  "extdata", "preprocessed_dds.rds", package = "GSEAlens"
...
# gsea_pathwaysets <- GSEAlens::build_gsea_pathways(
...
gsea_pathwaysets <- readRDS(system.file(
...
- **Combined Pathway Plotting**: aggregates multiple selected pathways into a
  ...
  single composite figure (uses `patchwork` under the hood).
...
- **DotPlot panel**: horizontal dot plot where the X axis is NES, dot color
  ...
  user-selectable), and dot size encodes gene-set magnitude. A fixed-domain

  `sqrt` size scale (rather than per-render min-max normalization) is used so

  that switching between "Core Genes Count" (leading edge) and "Set Size"

  (MSigDB gene-set total) produces visually distinct dot patterns and remains

  comparable across parameter changes. This mirrors the
...
a single composite canvas. The key feature is **one-click export of reproducible

R code**: the user can adjust the canvas parameters in the sidebar, then click

"Generate Code" to obtain a self-contained R script that reproduces the figure
...
Tabs 2, 3, and 6 include a **"Generate R Code"** button that produces a
