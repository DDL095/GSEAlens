...
- **Parallelization**: GSEAlens uses `r CRANpkg("future")` (`future::multisession`)
  ...
  on exit via `on.exit()`, so the global state is never polluted. The choice of
  `future` over `r Biocpkg("BiocParallel")` is benchmarked in the v0.99.7 release
  notes: for GSEAlens' typical workload that serializes more than 2 GB of globals
  (full DE table + gene set dictionary + metadata dictionary) on Windows,
  `future::multisession` was measured to be 3--5x faster than
  `BiocParallel::SnowParam` PSOCK serialization for this workload.
